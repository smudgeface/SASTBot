...
- **Repo-level reachability flags.** `repos.reachability_enabled` (default true) gates the LLM SCA reachability portion of the detection prompt. `repos.reachability_include_optional_deps` (default true) controls whether cdxgen `scope: "optional"` components are sent to the LLM as hints — kept as an opt-out because cdxgen's `optional` lumps devDependencies AND transitive runtime deps together, so excluding it risks false negatives. Whole feature is "Under review" — see `docs/M6_LLM_SAST_PLAN.md`.

- **Don't badge cdxgen `optional` as "DEV".** It's not a clean dev-only signal. Show the raw scope value (`runtime`, `optional`) where surfaced; if/when we add a real lockfile-based dev classifier, we can re-introduce a "DEV" badge with truthful semantics.
