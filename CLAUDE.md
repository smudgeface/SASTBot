...
- **SAST snippet match-line locator.** `latest_snippet` doesn't reliably have 3 lines of context above `latest_start_line` — the LLM emits anywhere from 0 to 20+ preamble lines. `ContextSnippet` searches the snippet for keywords from the issue summary (UPPER_SNAKE_CASE identifiers + ≥5-char content words with naive trailing-s stem) to locate the actual match line within the snippet, then renders 3 lines above the first hit + the contiguous span of all tied hits + 3 lines below. Falls back to offset-from-top only when no keywords match. Don't add new code that assumes a fixed offset between `latest_start_line` and snippet[0].
