...
**Next — M2**
GitHub repo creation (`smudgeface/SASTBot`) and first deploy to Dokploy (`10.0.0.0`). GitHub Actions for lint/test/build/typecheck + OpenAPI-drift check. Real BullMQ job: admin "Scan now" drives a `scan_runs` row through pending → running → done (still a no-op handler — M3 fills in the scan logic). Publish `docs/OPERATIONS.md` with key-rotation and webhook-deploy procedures.
